...
const getProducts = async (req, res, next) => {
    try {
        validateRequest(req);
        const metadata = await productStaffService.getProducts(req.user.id, req.query);
...
const getProductById = async (req, res, next) => {
    try {
        validateRequest(req);
        const metadata = await productStaffService.getProductById(req.user.id, Number(req.params.id));
